...
export function setupPhoneAuth(app: Express) {
  ...
  app.post("/api/auth/verify-otp", otpVerifyRateLimit, async (req, res) => {
    try {
      ...
      console.log(`✅ Phone Auth - User ${user.id} logged in with secure HTTP-only cookie`);
      ...
      res.json({ 
        success: true, 
        message: "Login successful with secure HTTP-only cookie",
        user: {
          id: user.id,
          phoneNumber: user.phoneNumber,
  ...
  app.post("/api/auth/logout", authenticateSession, async (req, res) => {
    try {
      ...
      res.json({ 
        success: true, 
        message: "Logged out successfully - JWT token revoked and cookie cleared",
        tokenRevoked: true
      });
    } catch (error) {
